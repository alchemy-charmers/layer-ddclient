...
# If layer options are used, add this to ddclient
# and import layer in lib_ddclient
@pytest.fixture
def mock_layers(monkeypatch):
    ...
    monkeypatch.setattr('lib_ddclient.layer.options', options)
...
def mock_hookenv_config(monkeypatch):
    ...
    monkeypatch.setattr('lib_ddclient.hookenv.config', mock_config)


@pytest.fixture
def mock_remote_unit(monkeypatch):
    monkeypatch.setattr('lib_ddclient.hookenv.remote_unit', lambda: 'unit-mock/0')


@pytest.fixture
def mock_charm_dir(monkeypatch):
    monkeypatch.setattr('lib_ddclient.hookenv.charm_dir', lambda: '/mock/charm/dir')
